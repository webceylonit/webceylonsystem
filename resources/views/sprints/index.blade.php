  ...
  <ul class="list-group">
    ...
    <li class="list-group-item d-flex justify-content-between align-items-center mb-2 p-3 shadow-sm"
      ...
      <div>
        ...
        <small>Start: {{ \Carbon\Carbon::parse($sprint->start_date)->format('d-m-Y') }} | End:
          <span style="color: {{ ($sprint->end_date < now() && $sprint->status != 'Completed') ? 'red' : 'inherit' }};">{{ \Carbon\Carbon::parse($sprint->end_date)->format('d-m-Y') }}</span>
        </small>
      </div>
