  ...
  <div class="page-title">
    <div class="row">
      ...
      <div class="col-6 text-end">
        ...
        @endpermission
        @permission('Create Tasks')
          <button class="btn btn-outline-info" data-bs-toggle="modal" data-bs-target="#addNoteHereModal">+ Add Note</button>
        @endpermission
      </div>
    </div>
  </div>
    ...
    @foreach ($board as $item)
      @if ($item['type'] === 'task')
        ...
        <li class="list-group-item d-flex justify-content-between align-items-center mb-1"
            data-type="t" data-id="{{ $task->id }}"
            style="border: {{ ($task->due_date && $task->due_date < now() && $task->status != 'Done') ? '2px solid red' : 'none' }};">
          <div class="d-flex align-items-start gap-2">
            <span class="drag-handle text-muted" title="Drag to reorder" style="cursor: grab; user-select: none;">☰</span>
            <div>
              <h6 class="mb-1">{{ $task->name }}</h6>
              <small>Assigned to: {{ optional($task->assignedTo)->name }}</small><br>
              <small>Status:
            ...
            <form method="POST" action="{{ route('tasks.update', $task->id) }}">
              ...
              <div class="modal-body">
                ...
                <div class="mb-3">
                  ...
                  <input type="date" class="form-control" name="start_date" value="{{ optional($task->start_date)->format('Y-m-d') }}">
                </div>
                <div class="mb-3">
      ...
      @elseif ($item['type'] === 'note')
        ...
        <li class="list-group-item mb-1"
            data-type="n" data-id="{{ $note->id }}"
            style="border-left: 6px solid {{ $note->color ?: '#17a2b8' }}; background: #f9fafb;">
          <div class="d-flex align-items-start gap-2">
            <span class="drag-handle text-muted" title="Drag to reorder" style="cursor: grab; user-select: none;">☰</span>
            <div class="flex-grow-1">
              <div class="d-flex justify-content-between">
                <div>
                  <span class="badge" style="background: {{ $note->color ?: '#17a2b8' }}">{{ $note->role ?: 'Note' }}</span>
                  @if($note->note_date)
                    <small class="text-muted ms-2">📅 {{ $note->note_date->format('Y-m-d') }}</small>
                  @endif
                </div>
                <div class="d-flex gap-1">
                  
                  <button class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#editNoteModal-{{ $note->id }}">Edit</button>
                  <form action="{{ route('sprint-notes.destroy', $note) }}" method="POST" onsubmit="return confirm('Delete this note?')">
                    @csrf @method('DELETE')
        ...
        <div class="modal fade" id="editNoteModal-{{ $note->id }}" tabindex="-1" aria-hidden="true">
          <div class="modal-dialog"><div class="modal-content">
            ...
            <form method="POST" action="{{ route('sprint-notes.update', $note) }}">
              @csrf @method('PUT')
              <div class="modal-body">
                <div class="row g-2">
                  <div class="col-md-6">
                  ...
                  <div class="col-md-3">
                    ...
                    <select class="form-control" name="role">
                      @foreach (['PM','Dev','QA','Design','Ops','Note'] as $r)
                        <option value="{{ $r }}" {{ $note->role===$r?'selected':'' }}>{{ $r }}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="col-md-3">
                    <label class="form-label">Color</label>
                    <input class="form-control" name="color" value="{{ $note->color ?: '#17a2b8' }}" placeholder="#ffeb3b">
                  </div>
                </div>
                <div class="row g-2 mt-2">
                  <div class="col-md-4">
                    <label class="form-label">Date</label>
                    <input type="date" class="form-control" name="note_date" value="{{ optional($note->note_date)->format('Y-m-d') }}">
                  </div>
                </div>
                <div class="mt-2">
                  <label class="form-label">Body</label>
                  <textarea class="form-control" rows="5" name="body">{{ $note->body }}</textarea>
                </div>
              </div>
              <div class="modal-footer">
      ...
      <div class="modal-body">
        ...
        <div class="mb-3"><label class="form-label">Status</label>
          <select class="form-control" name="status">
            ...
            <option value="Approval">To Approval</option>
...
<!-- Add Note (reusable; uses anchor token) -->
<div class="modal fade" id="addNoteHereModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog"><div class="modal-content">
    <div class="modal-header">
      <h5 class="modal-title">Add Note</h5>
      <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
    </div>
    <form method="POST" action="{{ route('sprint-notes.store') }}">
      @csrf
      <input type="hidden" name="sprint_id" value="{{ $sprint->id }}">
      <input type="hidden" name="insert_after" id="insertAfterToken" value="__END__">
      <div class="modal-body">
        <div class="row g-2">
          <div class="col-md-6">
            <label class="form-label">Title</label>
            <input class="form-control" name="title" placeholder="Eg: Today’s focus / Group A">
          </div>
          <div class="col-md-3">
            <label class="form-label">Role</label>
            <select class="form-control" name="role">
          ...
          <div class="col-md-3">
            <label class="form-label">Color</label>
            <input class="form-control" name="color" placeholder="#ffeb3b">
          </div>
        </div>
        <div class="row g-2 mt-2">
          <div class="col-md-4">
            <label class="form-label">Date</label>
            <input type="date" class="form-control" name="note_date" value="{{ now()->format('Y-m-d') }}">
          </div>
        </div>
        <div class="mt-2">
          <label class="form-label">Body</label>
          <textarea class="form-control" rows="5" name="body" placeholder="Notes, grouping, checkpoints..."></textarea>
        </div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // set anchor when clicking "+ Note here"
  document.querySelectorAll('.add-note-here').forEach(btn => {
    btn.addEventListener('click', function() {
      const token = this.getAttribute('data-anchor') || '__END__';
      document.getElementById('insertAfterToken').value = token;
    });
  });

  const token     = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
  const reorderUrl = @json(route('tasks.reorder'));
  const sprintId   = @json($sprint->id);

  function currentOrder() {
    return Array.from(list.querySelectorAll('li[data-type][data-id]'))
      .map(li => `${li.dataset.type}:${li.dataset.id}`);
  }
  ...
  new Sortable(list, {
    ...
    onEnd: saveOrder,
    onMove(evt) {
      if (evt.originalEvent?.target.closest('button, a, input, textarea, select, label, form')) {
        return false;
      }
    }
  });
